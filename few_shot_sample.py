...
examples = [
    ...
    'まあ、下宿より何となく派手で、居心地がいいのだろう。私はその晩も、例によって、一杯の冷しコーヒーを十分もかかって飲みながら、いつもの往来に面したテーブルに陣取って、ボンヤリ窓の外を眺めていた。さて、この白梅軒のあるＤ坂というのは、以前菊人形の名所だった所で、狭かった通りが、市区改正で取拡げられ、何間道路とかいう大通になって間もなくだから、まだ大通の両側に所々空地などもあって、今よりずっと淋しかった時分の話だ。大通を越して白梅軒の丁度真向うに、一軒の古本屋がある。'
]
...
def to_examples(arr):
    ...
    for i in range(0, len(arr), 2):
        examples.append(_get(arr, i) + ' => ' +  _get(arr, i+1))
    return examples

def gen_input(task, context, examples, prompt):
    e = to_examples(examples)
    b = task + '\n' + '\n'.join(e) + '\n' + prompt + '=>'    
    if context is None:
        return b
    return context + '\n' + b
